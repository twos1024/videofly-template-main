...
export class KieProvider implements AIVideoProvider {
  ...
  async createTask(params: VideoGenerationParams): Promise<VideoTaskResponse> {
    ...
    const hasMappedConfig = providerConfig?.supported === true;
    const apiEndpoint = providerConfig?.apiEndpoint || "/api/v1/jobs/createTask";

    const providerModelId = hasMappedConfig
      ? getProviderModelId(internalModelId, "kie", params)
      : internalModelId;
    const transformedParams = hasMappedConfig
      ? transformParamsForProvider(internalModelId, "kie", params)
      : this.transformGenericParams(params);
  ...
  private transformGenericParams(
