    ...
    <section className="container py-12 md:py-20">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as PricingTab)} className="w-full">
        <TabsList className="mx-auto grid max-w-2xl grid-cols-3">
          <TabsTrigger value="onetime">{dictPrice.tab_onetime}</TabsTrigger>
          <TabsTrigger value="monthly">{dictPrice.tab_monthly}</TabsTrigger>
          <TabsTrigger value="yearly" className="relative">
            {dictPrice.tab_yearly}
            <Badge className="absolute -top-2 -right-2 h-5 px-2 bg-destructive text-xs">
              {dictPrice.off_percent}
            </Badge>
          </TabsTrigger>
        </TabsList>
      ...
      <p className="mt-16 text-center text-base text-muted-foreground">
        <Balancer>
          ...
          <a
            ...
            href={`mailto:${dictPrice.support_email}`}
          >
            {dictPrice.support_email}
          </a>{" "}
          {dictPrice.contact}
          <br />
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  {product.billingPeriod && (
                    <span className="text-muted-foreground text-sm">
                      /{product.billingPeriod === "year" ? dictPrice.per_year?.replace("/", "") : dictPrice.per_month?.replace("/", "")}
                    </span>
                  )}
                </div>
              </div>
