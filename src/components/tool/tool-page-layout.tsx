...
export function ToolPageLayout({
  ...
  const handleFailed = useCallback(
    ({ videoId, error }: { videoId: string; error?: string }) => {
      ...
      if (shouldNotify(notificationKey)) {
        const message = error || tTool("errors.generateFailed");
        toast.error(message);
  ...
  useEffect(() => {
    ...
    if (handledQueryVideoIdRef.current === videoIdFromQuery) return;
    handledQueryVideoIdRef.current = videoIdFromQuery;

    // 立即添加到历史记录（即使是正在生成中）
    const existingItem = videoHistoryStorage
      .getHistory(user.id)
      .find((item) => item.uuid === videoIdFromQuery);
  ...
  const handleSubmit = useCallback(async (data: GeneratorData) => {
    ...
    try {
      ...
      toast.success(tTool("messages.generationStarted"));
      ...
      // 显示错误提示
      toast.error(error instanceof Error ? error.message : tTool("errors.generateFailed"));
    }
  ...
  const handleDelete = useCallback(async (uuid: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(tTool("errors.deleteFailed"));
      }
      ...
      toast.success(tTool("messages.deleteSucceeded"));
    } catch (error) {
      console.error("Delete error:", error);
      toast.error(tTool("errors.deleteFailed"));
    }
  }, [user?.id, tTool]);
  ...
  const handleRetry = useCallback(async (uuid: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(tTool("errors.retryFailed"));
      }
      await response.json();
      ...
      toast.success(tTool("messages.retryStarted"));
    } catch (error) {
      console.error("Retry error:", error);
      toast.error(tTool("errors.retryFailed"));
    }
              ...
              <div className={`flex flex-col lg:flex-row gap-6 ${activeTab === "generator" ? "" : "lg:flex"}`}>
                ...
                <div className={`${activeTab === "result" ? "block" : "hidden"} lg:block flex-1 min-h-[500px] rounded-2xl border border-border bg-muted/20 overflow-hidden relative`}>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 text-muted-foreground">
                    ...
                    <h3 className="text-lg font-medium mb-2">{tTool("guestPreviewTitle")}</h3>
                    <p className="text-sm max-w-xs">{tTool("guestPreviewDescription")}</p>
                  </div>
                </div>
              </div>
            </div>
