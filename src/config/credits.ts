...
export const CREDITS_CONFIG = {
  ...
  // ========== 图片模型配置（从 pricing-user.ts 生成）==========
  imageModels: Object.fromEntries(
    Object.entries(IMAGE_MODEL_PRICING)
      .filter(([_, pricing]) => pricing.enabled)
      .map(([modelId, pricing]) => {
        const imageBaseConfigs: Record<
          string,
          Omit<ImageModelConfig, "creditCost">
        > = {
          "flux-1": {
            id: "flux-1",
            name: "Flux 1",
          ...
          "dall-e-3": {
            id: "dall-e-3",
            name: "DALL-E 3",
            description: "Advanced text-to-image with exceptional detail",
            aspectRatios: ["1:1", "16:9", "9:16"],
            provider: "kie",
          },
        };

        const baseConfig = imageBaseConfigs[modelId];
        if (!baseConfig) return null;

        return [modelId, { ...baseConfig, creditCost: pricing.baseCredits }];
      })
      .filter((entry): entry is [string, ImageModelConfig] => entry !== null)
...
export function getAvailableImageModels(): ImageModelConfig[] {
  return Object.values(CREDITS_CONFIG.imageModels);
}
