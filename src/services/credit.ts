...
export class CreditService {
  ...
  async freeze(params: {
    ...
    return db.transaction(async (trx) => {
      await trx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtext(${`credit_freeze:${userId}`}))`
      );
  ...
  async settle(videoUuid: string): Promise<void> {
    await db.transaction(async (trx) => {
      await this.settleInTx(trx, videoUuid);
    });
  }
  ...
  async release(videoUuid: string): Promise<void> {
    await db.transaction(async (trx) => {
      await this.releaseInTx(trx, videoUuid);
    });
  }
  ...
  async recharge(params: {
    ...
    return db.transaction(async (trx) => {
      await trx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtext(${`credit_recharge:${params.orderNo}`}))`
      );

      const [existingPkg] = await trx
      ...
      if (existingPkg) {
        return { packageId: existingPkg.id };
      }
