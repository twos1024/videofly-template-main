...
const stripeClient = stripeApiKey
  ? new Stripe(stripeApiKey, { typescript: true })
  : null;

const missingStripeClient = new Proxy(
  {},
  {
    get() {
      throw new Error(
        "Stripe is not configured. Set STRIPE_API_KEY or switch NEXT_PUBLIC_BILLING_PROVIDER to creem."
      );
    },
  }
) as Stripe;

export const stripe = stripeClient ?? missingStripeClient;
